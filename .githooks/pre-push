...
check_prerequisites() {
  debug_log "Init check_prerequisites()"
...
validate_json_file() {
  debug_log "Init validate_json_file()"
  local file="$1"
  if ! jq empty "$file" &> /dev/null; then
    log "[ERROR]" "Invalid JSON in file: $file. Contents: $(cat "$file")"
    exit 1
  fi
}
...
read_config() {
  ...
  if [ -f "$CONFIG_FILE" ]; then
    ...
    if [ -z "$DRY_RUN" ]; then
      log "[ERROR]" "Missing key 'environment.dryrun' in $CONFIG_FILE."
      exit 1
    fi
    ...
    log "[INFO]" "Config read: resumetype='$TARGET_RESUMETYPE', language='$TARGET_LANGUAGE', dryrun='$DRY_RUN', debug_mode='$DEBUG_MODE'"
  else
    log "[WARNING]" "config.json not found. Defaulting to resumetype: master, language: no, dryrun: 0, debug: 1"
...
find_resume_file() {
  debug_log "Init find_resume_file()"
  FOUND_SOURCE_FILE=""
...
if [ ! -f "$LOG_FILE" ]; then
  ...
  log "[INFO]" "Log file already exists at $LOG_FILE truncating." To avoid filling up the disk space.
  > "$LOG_FILE"
  log "[INFO]" "Log file truncated."
fi
